...
namespace PluginTest
{
    ...
    {
        [CommandMethod("MyCreatePolilne")]
        public void MyCreatePolilne()
        {
            Document doc = Application.DocumentManager.MdiActiveDocument;
            Database db = doc.Database;
            Editor ed = doc.Editor;

            //tworzymy nowa transakcje bazy danych
            //podczas transakcji mozemy tworzyc, edytowac i niszczyc obiekty 
            //znajdujace sie w bazie danych projektu
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                //otwieramy BlockTable w trybie do odczytu
                BlockTable blockTable;
                blockTable = tr.GetObject(db.BlockTableId, OpenMode.ForRead) as BlockTable;

                // Otwieramy rekord BlockTable tylko do odczytu w przestrzeni modelu
                BlockTableRecord blockTableRecord;
                blockTableRecord = tr.GetObject(blockTable[BlockTableRecord.ModelSpace], OpenMode.ForWrite) as BlockTableRecord;

                // Tworzymy polylinie tworzaca kwadrat
                using (Polyline polyline = new Polyline())
                {
                    //new Point2d().TransformBy(Matrix2d.Rotation())
                    //dodajemy kolejne werteksy
                    polyline.AddVertexAt(0, new Point2d(0, 0), 0, 0, 0);
                    polyline.AddVertexAt(1, new Point2d(0, 5), 0, 0, 0);
                    polyline.AddVertexAt(2, new Point2d(5, 5), 0, 0, 0);
                    polyline.AddVertexAt(3, new Point2d(5, 0), 0, 0, 0);
                    polyline.AddVertexAt(4, new Point2d(0, 0), 0, 0, 0);
                    
                    //nowo stworzona polylinie dodajmy do rekordu BlockTable
                    blockTableRecord.AppendEntity(polyline);
                    //dodajemy tworzenie nowego obiektu do transakcji bazy danych
                    tr.AddNewlyCreatedDBObject(polyline, true);
                }

                //wykonujemy transakcje, co skutkuje zapisem nowego obeiktu do bazy danych 
                tr.Commit();
            }
        }

        [CommandMethod("MyEditPolyline")]
        public void MyEditPolyline()
        {
            Document doc = Application.DocumentManager.MdiActiveDocument;
            Editor ed = doc.Editor;
            Database db = doc.Database;

            //prosimy o pobranie polylini
            PromptEntityResult entityResult = ed.GetEntity("Select a polyline");

            //jesli wybrano obiekt
            if (entityResult.Status == PromptStatus.OK)
            {
                //zaczynamy nowa transakcje
                Transaction tr = db.TransactionManager.StartTransaction();
                using (tr)
                {
                    //otwieramy wybrany obiekt do zapisu
                    DBObject obj = tr.GetObject(entityResult.ObjectId, OpenMode.ForWrite);
                    //i probujemy rzutowac go na polylinie
                    Polyline lwp = obj as Polyline;
                    //jesli lwp jest rozne od nulla, to oznacza, ze obiekt jest polylinia
                    if (lwp != null)
                    {
                        //sprawdzamy ilosc werteksow w polylini
                        int vn = lwp.NumberOfVertices;
                        for (int i = 0; i < vn; i++)
                        {
                            //wyswietlamy kolejne punkty polylini do konsoli edytora
                            Point2d pt = lwp.GetPoint2dAt(i);
                            ed.WriteMessage("\n" + pt.ToString());
                        }

                        //obracamy wybrana polylinie o 90 stopni (podane w radianach),
                        //wokol osi Z z srodkiem obrotu w punkcie (0,0,0)
                        lwp.TransformBy(Matrix3d.Rotation(Math.PI/2, Vector3d.ZAxis, new Point3d(0, 0, 0)));

                    }
                    //zapisujemy zmiany polylini
                    tr.Commit();
